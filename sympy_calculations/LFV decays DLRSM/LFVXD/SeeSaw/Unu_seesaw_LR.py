'''
    Definitions involved in neutrino masses in SeeSaw model in the context of MLRM
...
mn = Matrix([[mn1, 0, 0], [0, mn2, 0], [0, 0, mn3]])
mN = Matrix([[mn4, 0, 0], [0, mn5, 0], [0, 0, mn6]])


I3 = eye(3, 3)

vL, vR = symbols('v_L, v_R', real=True)
R2 = vL/vR*I3-mn*mN.inv()
R = Matrix(
    [
        [sqrt(vL/vR-mn1/mn4), 0, 0], 
        [0, sqrt(vL/vR-mn2/mn3), 0], 
        [0, 0, sqrt(vL/vR-mn3/mn6)]
    ]
    )
MD = conjugate(Upmns)*mN*R*conjugate(Upmns.T) #### Agregar R
MDT = MD.T
...
def MD_mp(mn1, mn2, mn3, mn4, mn5, mn6):
    ...
    '''
    return 1j*Upmns_mp*MNdsqrt_mp(mn4, mn5, mn6)*mndsqrt_mp(mn1, mn2, mn3) #### Agregar R
