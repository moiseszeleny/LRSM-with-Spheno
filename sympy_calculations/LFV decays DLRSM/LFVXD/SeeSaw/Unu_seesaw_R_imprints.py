...
from sympy import symbols, conjugate, Matrix, sqrt, I, eye, sin, cos
...
th1, th2, th3 = symbols(r'\theta_{1},\theta_{2},\theta_{3}', real=True)
s1, s2, s3 = sin(th1), sin(th2), sin(th3)
c1, c2, c3 = cos(th1), cos(th2), cos(th3)
...
#MD = I*conjugate(Upmns)*MNdsqrt*mndsqrt #### Agregar R
MD = MNdsqrt*R_sp*mndsqrt*Upmns.T
MDT = MD.T
...
# #################
def MD_mp(mn1, mn2, mn3, mn4, mn5, mn6, th1, th2, th3):
    '''
        Dirac neutrino mass matrix in the SeeSaw model.
    '''#1j
    return 1j*MNdsqrt_mp(mn4, mn5, mn6)*R_mp(th1, th2, th3)*mndsqrt_mp(mn1, mn2, mn3)*Upmns_mp.T #### Agregar R


# ###### Diagonalizando con mpmath
def MD_NO(m1, m2, m3, m4, m5, m6, th1, th2, th3):
    '''
        Dirac neutrino mass matrix in the SeeSaw model in the Normal Order.
    '''
    return MD_mp(m1, m2, m3, m4, m5, m6, th1, th2, th3)
...
def Mnu(m1, m2, m3, m4, m5, m6, th1, th2, th3):
    '''
        Neutrino mass matrix in the SeeSaw model.
    '''
    M = MD_NO(m1, m2, m3, m4, m5, m6, th1, th2, th3)
    # print(M[0,0])
    return mp.matrix([
...
def diagonalizationMnu(m1, m2, m3, m4, m5, m6, th1, th2, th3):
    '''
        Diagonalization of neutrino mass matrix in the SeeSaw model.
    '''
    Mi, UL, UR = mp.eig(Mnu(m1, m2, m3, m4, m5, m6, th1, th2, th3), left=True, right=True)
    Mi, UL, UR = mp.eig_sort(Mi, UL, UR)
    return Mi, UL, UR


def diagonalizationMnu_svd(m1, m2, m3, m4, m5, m6, th1, th2, th3):
    '''
        Diagonalization of neutrino mass matrix in the SeeSaw model using the
        singular value descomposition method.
    '''
    UL, Mi, UR = mp.svd_c(Mnu(m1, m2, m3, m4, m5, m6, th1, th2, th3))
    return Mi, UL, UR
...
def MD_np(mn1, mn2, mn3, mn4, mn5, mn6):
    ...
    '''
    return 1j*Upmns_np*MNdsqrt_np(mn4, mn5, mn6)*mndsqrt_np(mn1, mn2, mn3) #### Agregar R
