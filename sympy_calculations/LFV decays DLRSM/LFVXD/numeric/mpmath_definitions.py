...
def C0(mX, mi, mj, M0, M1, M2):
    ...
    y2 = x2(mX, M1, M2)
    y3 = x3(M0, M1)
    return ((R0(y0, y1) + R0(y0, y2) - R0(y0, y3))/mX**2)  # *(-1j*16)


# @jit
def C1(mX, mi, mj, M0, M1, M2):
    '''
    C1 PV function
    ...
    return (
        (1/mX**2)*(
            B1_0(mi, M0, M1) -
            B12_0(mX, M1, M2) +
            (M2**2 - M0**2)*C0(mX, M0, M1, M2)
            )
        )


# @jit
def C2(mX, mi, mj, M0, M1, M2):
    '''
    C2 PV function
